...
(function($) {
    ...
    woonWidget = function(element, options){
        ...
        ns.init = function() {
            ...
            zc_input.unbind('focus').bind('focus', function() {
				ns.initWidget('#postcode');
			});
			
            $(".widget-opener input[name='huisnummer']").unbind('focus').bind('focus', function() {
				ns.initWidget('#huisnummer');
			});
            $(".widget-opener input[name='toevoeging']").unbind('focus').bind('focus', function() {
				ns.initWidget('#toevoeging');
			});
			
			
            $("#toevoeging").unbind('keyup').bind('keyup', function() {
				$(".widget-opener input[name='toevoeging']").val($(this).val());
			});
			
            $("#huisnummer").unbind('keyup').bind('keyup', function() {
				$(".widget-opener input[name='huisnummer']").val($(this).val());
			});			
	        
			$("#postcode").unbind('keyup').bind('keyup', function() {
				$(".widget-opener input[name='postcode']").val($(this).val());
			});		
			


            $('#tooltip_housetype').tooltipster({
                content: $("#tooltip_housetype").attr('data-tooltip'),
                // setting a same value to minWidth and maxWidth will result in a fixed width
                minWidth: 250,
                maxWidth: 250
            });

            $('#tooltip_houserooms').tooltipster({
                content: $("#tooltip_houserooms").attr('data-tooltip'),
                minWidth: 250,
                maxWidth: 250
            });
        ...
        ns.initWidget = function (focusTarget) {
            ...
            if (!$('body > .semi-layer').length) {
                ...
                $('#house-insurance-form ' + focusTarget).focus();
				$('#house-insurance-form ' + focusTarget).select();
            }
            ...
            $("#submit_woon_form").click(function(e) {
                $('#house-insurance-form .error').removeClass('error');
                ...
                if ($.isEmptyObject(errorMessages)) {
                    ...
                    $.each(errorMessages, function(errorMessageKey, errorMessage) {
                        ...
                        $('#house-insurance-form ' + '#' + errorMessageKey).addClass('error');
        ...
        ns.validateWoonForm = function() {
            ...
            housenumber = $('#house-insurance-form #huisnummer').val();
            zipcode = $('#house-insurance-form #postcode').val();
            extension = $('#house-insurance-form #toevoeging').val();
            birthdate = $('#house-insurance-form #birthdate').val();
            houseType = $('#house-insurance-form select[name=housetype]').val();
            roofTopType = $('#house-insurance-form input:radio[name=rooftype]:checked').val();
            nrOfRooms = $('#house-insurance-form #houserooms').val();
            isOwner = $('#house-insurance-form input:radio[name=houseowner]:checked').val();
    ...
    var theCRE = $("#submit_woon_form").closest('form').attr('data-cre');
